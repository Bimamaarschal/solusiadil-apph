...
function validateUU(input) {
  if (input.value.length > 550) {
      ...
      document.getElementById('error-message-undang_undang').style.display = 'block';
  } else {
      document.getElementById('error-message-undang_undang').style.display = 'none';
  }
}
function validateReferensi(input) {
  if (input.value.length > 550) {
      input.value = input.value.slice(0, 550);
      document.getElementById('error-message-doc_pendukung').style.display = 'block';
  } else {
      document.getElementById('error-message-doc_pendukung').style.display = 'none';
  }
}
...
document.addEventListener("DOMContentLoaded", function() {
    document.getElementById("id_blog").value = generateRandomID();
});
